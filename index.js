...
function getTableOffensive(elements) {
  ...
  for(const key of keys) 
  {
    ...
    // Get element weaknesses/resistances
    const weaknesses = getTypeWeaknesses(key);
    const resistances = getTypeResistances(key);
    ...
    for(const element of elements) {
      ...
      else if (resistances.includes(element)) {
        map[index][1]++;
      }
      else { // Neutral
        map[index][2]++;
...
function getTableSkills(skills) {
    ...
    for(const index in SKILLS) {
      ...
      if (Object.keys(skills).includes(key)) {
        ...
        // Loop over the table skills
        for(const level of skills[key]) {
          // Increment the level
          map[index][level]++;
        }
      }
